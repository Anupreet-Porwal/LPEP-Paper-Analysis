...
# Calculate posterior probabilities of each model in model space
...
# Calculate posterior probabilities for each model in model space for BAS method
...
#Calculate 95% credible intervals based on posterior samples for coefficients
...
#load the dataset
load(url("https://github.com/Anupreet-Porwal/LPEP/blob/master/data/endometrial.RData?raw=TRUE"))
...
y <- endometrial$HG
x <- as.matrix(subset(endometrial,select = -HG))
mydata <- endometrial
...
res.endometrial <-list("PIP"=pip.mat,
                       "coef" = coef.mat,
                       "model.probs"= model.list,
                       "credible.int"=ci.methods)

# Save results
save(res.endometrial,file = "endometrial_res.Rda")
